...
export class SlimeEnemy extends EnemyUnit {
  ...
  private updateDash(delta: number): void {
    ...
    if (this.isDashing) {
      ...
      if (target && Math.random() < 0.01) { // 1%の確率でダッシュ判定
  ...
  updateAI(delta: number): void {
    if (this.isDashing) {
      // ダッシュ中は方向に沿って移動
      this.x += this.dashDirection.x * this.defaultSpeed * this.dashSpeedMultiplier * (delta / 1000);
      this.y += this.dashDirection.y * this.defaultSpeed * this.dashSpeedMultiplier * (delta / 1000);
